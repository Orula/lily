...
class ANN:
  ...
  def optimize(self, batch_size, lr=0.8):
    ...
    ##### Momentum + SDG update.
    lr, b1, b2, eps = 0.8, 0.9, 0.999, 1e-8
    self.grads["m b"] = [np.zeros(b.shape) for b in self.params["b"]]
    self.grads["m w"] = [np.zeros(w.shape) for w in self.params["w"]]

    self.grads["m b"] = [ b1*m + (1-b1)*g for m, g in zip(self.grads["m b"], self.grads["b"]) ]
    self.grads["m w"] = [ b1*m + (1-b1)*g for m, g in zip(self.grads["m w"], self.grads["w"]) ]

    self.params["b"] = [b-(lr/batch_size)*nb for b, nb in zip(self.params["b"], self.grads["m b"])]
    self.params["w"] = [w-(lr/batch_size)*nw for w, nw in zip(self.params["w"], self.grads["m w"])]

    ##### ADAM update rule  -- NOT working
    #lr, b1, b2, eps = 0.8, 0.9, 0.999, 1e-8
    #self.grads["m b"] = [np.zeros(b.shape) for b in self.params["b"]]
    #self.grads["m w"] = [np.zeros(w.shape) for w in self.params["w"]]
    #self.grads["rmsprop b"] = [np.zeros(b.shape) for b in self.params["b"]]
    #self.grads["rmsprop w"] = [np.zeros(w.shape) for w in self.params["w"]]
    #### Momentum 
    #self.grads["m b"] = [ b1*m + (1-b1)*g for m, g in zip(self.grads["m b"], self.grads["b"]) ]
    #self.grads["m w"] = [ b1*m + (1-b1)*g for m, g in zip(self.grads["m w"], self.grads["w"]) ]

    #### RMS
    #self.grads["rmsprop b"] = [b2*m + (1-b2)*np.power(g,2) for m, g in zip(self.grads["rmsprop b"], self.grads["b"]) ]
    #self.grads["rmsprop w"] = [b2*m + (1-b2)*np.power(g,2) for m, g in zip(self.grads["rmsprop w"], self.grads["w"]) ]

    #self.params["b"] = [b-(lr/batch_size)*(1/(np.sqrt(rms) +eps))*g for b, rms, g in zip(self.params["b"],self.grads["rmsprop b"], self.grads["m b"])]
    #self.params["w"] = [w-(lr/batch_size)*(1/(np.sqrt(rms) +eps))*g for w, rms, g in zip(self.params["w"],self.grads["rmsprop w"], self.grads["m w"])]
